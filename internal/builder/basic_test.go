...
// checks it against generatedGoHeader rather than against a copy of that text:
// the constant is what decides whether generation may overwrite or prune a
// file it finds on disk, so asserting a separate literal here would let the
// two drift apart while both tests stayed green. The last assertion runs the
// artifact back through the ownership check itself, which is the same link
// TestOwnedGeneratedFileRecognizesRenderedOutput pins for rendered output.
func assertGeneratedGoHeader(t *testing.T, dir string) {
	t.Helper()

	path := filepath.Join(dir, "jsonschema_gen.go")
	generated, err := os.ReadFile(path)
	require.NoError(t, err)
	require.True(t, strings.HasPrefix(string(generated), "//go:build !jsonschema\n\n"))
	require.Contains(t, string(generated), builder.GeneratedGoHeader+"\n\npackage ")
	require.NotContains(t, string(generated), "// +build")

	owned, err := builder.OwnedGeneratedFile(path)
	require.NoError(t, err)
	require.True(t, owned, "generation does not recognize its own output as owned")
}
