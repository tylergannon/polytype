...
// schemaAccessorData is the generated file of a package whose root has a
// schema entrypoint: the embedded FS and one accessor beside the codecs.
func schemaAccessorData() schemaTemplateData {
	data := rootCodecData()
	data.GenerateSchemas = true
	data.SchemaMethods = []SchemaAccessor{{TypeName: "Person", MethodName: "Schema"}}
	return data
}

// TestOwnedGeneratedFileRecognizesRenderedOutput pins generatedGoHeader to the
// header schemas.go.tmpl actually emits.
//
// The constant is what ownedGeneratedFile matches to decide whether a file
// already on disk is polytype's to overwrite or prune, and nothing else in the
// tree compares it to the template. Edit the header in one place only and
// generation silently stops recognizing its own previous output: it leaves the
// other generated file orphaned instead of removing it. Rendering the template
// and requiring ownedGeneratedFile to accept the result is the link that makes
// that drift fail here.
func TestOwnedGeneratedFileRecognizesRenderedOutput(t *testing.T) {
	for _, tc := range []struct {
		name string
		data schemaTemplateData
	}{
		{name: "codec_only", data: rootCodecData()},
		{name: "with_schemas", data: schemaAccessorData()},
	} {
		t.Run(tc.name, func(t *testing.T) {
			code := render(t, tc.data)
			path := filepath.Join(t.TempDir(), syntax.GeneratedSchemaFile)
			require.NoError(t, os.WriteFile(path, []byte(code), 0644))

			owned, err := ownedGeneratedFile(path)
			require.NoError(t, err)
			require.True(t, owned,
				"rendered output is not recognized as polytype's own: generatedGoHeader (%q) has drifted from the header in schemas.go.tmpl:\n%s",
				generatedGoHeader, code)
		})
	}
}

// TestOwnedGeneratedFileRejectsWhatPolytypeDidNotWrite proves the recognition
// above is a real match and not a function that says yes to everything: a
// handwritten file, another tool's generated file, a header that only appears
// below the package clause, and a file that is not there at all are all
// disowned, so polytype never removes a file it did not write.
func TestOwnedGeneratedFileRejectsWhatPolytypeDidNotWrite(t *testing.T) {
	for _, tc := range []struct {
		name string
		src  string
	}{
		{name: "handwritten", src: "package fixture\n\nfunc Hello() {}\n"},
		{name: "another_tool", src: "// Code generated by stringer. DO NOT EDIT.\n\npackage fixture\n"},
		{name: "header_below_package_clause", src: "package fixture\n\n" + generatedGoHeader + "\nfunc Hello() {}\n"},
	} {
		t.Run(tc.name, func(t *testing.T) {
			path := filepath.Join(t.TempDir(), syntax.GeneratedSchemaFile)
			require.NoError(t, os.WriteFile(path, []byte(tc.src), 0644))
	...
	t.Run("missing", func(t *testing.T) {
		owned, err := ownedGeneratedFile(filepath.Join(t.TempDir(), syntax.GeneratedSchemaFile))
		require.NoError(t, err)
		require.False(t, owned)
	})
}
