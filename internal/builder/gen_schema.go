...
func (s *SchemaBuilder) RenderGoCode() (err error) {
	...
	// The file is named for what it holds. The other name is removed, so
	// switching between schema and codec-only output never leaves two
	// generated files declaring the same methods.
	...
	return removeGeneratedFile(filepath.Join(s.Scan.Pkg.Dir, other))
}

// removeGeneratedFile deletes path if it holds generated Go code, leaving a
// handwritten file of that name alone.
func removeGeneratedFile(path string) error {
	...
	if err != nil {
		return fmt.Errorf("inspect stale generated file: %w", err)
	}
