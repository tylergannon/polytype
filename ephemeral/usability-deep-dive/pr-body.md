...
This change makes declarations ordinary executable configuration values and adds the public `codegen` package. `Declare[T]()` selects a type without implying a schema method, while `Declare(T.Schema)` retains the traditional accessor identity. Callers can compose declarations and sealed-union settings, retain field identity and provider type safety with `Field[Owner, ValueType]("Name")`, and independently select JSON Schema, Go JSON codecs, TypeScript, or devalue output. Build-tagged declarations use the same executable API.
